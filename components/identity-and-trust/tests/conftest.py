...
from koa_identity_and_trust.api import CONTRACT_VERSION, RequestContext, build_router  # noqa: E402
...
def headers_for(spec, *, correlation_id: str | None = None) -> dict[str, str]:
    headers = {
        ...
        "x-koa-correlation-id": correlation_id or f"corr:{spec.operation_id}:test",
    }
...
def contract_path(repository_root: Path) -> Path:
    return repository_root / "docs/contracts/components/identity-and-trust.component.json"
...
def complete_service():
    ...
    return FakeService({op: response_for(spec) for op, spec in OPERATIONS.items()})


@pytest.fixture
def router(complete_service):
