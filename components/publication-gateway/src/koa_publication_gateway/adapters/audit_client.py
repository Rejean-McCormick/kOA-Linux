...
class AuditClient:
    """Submit bounded publication evidence without direct Audit Broker writes."""
    ...
    _ALLOWED_EVIDENCE_TYPES = frozenset(
        {
            ...
            "publication_partially_delivered",
            "publication_failed",
            "publication_cancelled",
            "publication_withdrawn",
            "publication_remediated",
            "publication_retry_blocked",
    ...
    def submit_evidence(
        ...
        *,
        ...
        body = {
            ...
            "event_class_id": f"publication_gateway.{kind}",
            "producer_component_id": "publication_gateway",
            ...
            "source_receipt_or_evidence_refs": [
                self._text(item, "source_receipt_or_evidence_ref")
                for item in source_receipt_or_evidence_refs
            ],
            "policy_or_contract_ref": self._text(
                policy_or_contract_ref, "policy_or_contract_ref"
            ),
        ...
        if not isinstance(receipt_ref, str) or not receipt_ref.strip():
            raise ProtocolError("audit submission audit_receipt_ref is invalid")
        if not isinstance(accepted_at, str):
            raise ProtocolError("audit submission accepted_at is invalid")
        try:
        ...
        return AuditSubmission(
            ...
            audit_receipt_ref=receipt_ref.strip(),
            accepted_at=parsed.astimezone(timezone.utc),
