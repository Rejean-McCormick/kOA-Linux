...
SRC = ROOT / "integrations/orgo/adapter/src"
PACKAGE = "koa_orgo_adapter"
CONTRACT = ROOT / "docs/contracts/subsystems/orgo.subsystem.json"
DEPENDENCY = "B-0066"
EXPECTED_MODULES = ('bootstrap', 'client', 'health', 'capabilities', 'receipts', 'tasks', 'commands', 'surface_bridge')
...
def test_canonical_subsystem_boundary() -> None:
    ...
    assert contract["subsystem_id"] == "orgo"
    ...
    assert "task, organization, scheduling" in contract["koa_role"]
...
def test_declared_specialized_modules_exist_and_do_not_vendor_the_subsystem() -> None:
    ...
    assert "subsystems.orgo" not in text
